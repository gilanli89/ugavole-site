import { fresh, move, step, showDocuments, type GameState } from "./game";
...
export function validateReplay(
  value: unknown,
): { frames: number; inputs: ReplayInput[] } | null {
...
export function replayRun(
  ...
  for (let frame = 0; frame < frames; frame++) {
    ...
    while (next < inputs.length && inputs[next][0] === frame)
      applyInput(state, inputs[next++][1]);
