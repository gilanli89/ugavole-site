  ...
  return (
    <div className="controls-row" aria-label="Sürüş kontrolleri">
      <div className="steering-group" role="group" aria-label="Direksiyon">
        <div className="control-caption">
          ...
          <span
            ...
            aria-label={
              state.lane === 0 ? "Emniyet şeridi" : `${state.lane}. şerit`
            }
          >
            {[0, 1, 2].map((lane) => (
              <i key={lane} data-current={state.lane === lane} />
            ))}
          </span>
        </div>
        <div className="steering-buttons">
          {([-1, 1] as const).map((direction) => (
            <button
              ...
              onClick={(event) => {
                if (event.detail === 0) onSteer(direction);
              }}
      ...
      <div className="pedal-group" role="group" aria-label="Hız ve durma">
        <div className="control-caption">
          <span>HIZ KONTROLÜ</span>
          <small>Basılı tut</small>
        </div>
        <div className="pedal-buttons">
          {(["brake", "throttle"] as const).map((pedal) => (
            <button
              ...
              aria-label={
                pedal === "brake" ? "Fren yap ve dur" : "Gaz ver ve hızlan"
              }
              aria-keyshortcuts={
                pedal === "brake" ? "ArrowDown S Space" : "ArrowUp W"
              }
              ...
              onPointerUp={(event) =>
                onPedal(pedal, false, `pointer:${event.pointerId}`)
              }
              ...
              onClick={(event) => {
                ...
                if (event.detail === 0)
                  onPedal(pedal, !state[pedal], "assistive");
              }}
              onContextMenu={(event) => event.preventDefault()}
            >
              {pedal === "brake" ? (
                ...
                <ChevronsUp size={30} strokeWidth={2.8} />
              )}
      ...
      <p className="keyboard-hint">
        <kbd>←</kbd>
        <kbd>→</kbd> yön <kbd>↑</kbd> gaz <kbd>↓</kbd> / <kbd>BOŞLUK</kbd> fren
      </p>
    </div>
  );
}
